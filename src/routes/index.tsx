  ...
  head: () => ({
    meta: [
      { title: "Lumen — Turn messy text into beautiful documents" },
      {
        name: "description",
        content:
          "Paste raw, unstructured text. Get back a beautifully formatted, editable document with 22 themes and 11 export formats.",
      },
    ],
  }),
});
    ...
    async (extra?: string) => {
      ...
      try {
        ...
        const directives = [
          ...
          docType !== "auto" &&
            `Document type: ${dt?.label}; adapt labels only, not the shared template sequence.`,
          `Tone: ${tone}.`,
  ...
  const applyFormat = (kind: string) => {
    ...
    switch (kind) {
      ...
      case "hr":
        ...
        break;
      case "bold":
        md = md.replace(/\b(\w{4,})\b/, (m) => wrap(m, "**"));
        break;
      case "italic":
        md = md.replace(/\b(\w{4,})\b/, (m) => wrap(m, "_"));
        break;
      case "callout-tip":
        md =
          md +
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
                  brand document refinery
                </div>
              </div>
            </div>
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              <span className="hidden sm:inline">
                {DOC_TYPES.length} doc types · {THEMES.length} themes · {PAGE_SIZES.length} sizes ·
                11 exports
              </span>
            </div>
          </div>
        ...
        <section className="relative mx-auto max-w-5xl px-6 pb-8 pt-10 text-center">
          ...
          >
            ...
            <p className="mx-auto mt-4 max-w-2xl text-base text-muted-foreground">
              Pick a document type and a theme, refine, then customise every page detail — size,
              borders, colors, header & footer — before exporting.
            </p>
          </motion.div>
        </section>
      )}
            ...
            {showSource && (
              <div className="border-t border-border p-5">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  <SmallBtn onClick={handlePaste} icon={<ClipboardPaste className="h-3.5 w-3.5" />}>
                    Paste
                  </SmallBtn>
                  <SmallBtn
                  ...
                  <input
                    ref={fileRef}
                    type="file"
                    accept=".txt,.md,.markdown,.docx,.text,.rtf"
                    className="hidden"
                    onChange={(e) => {
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 pt-1">
                      ...
                      {[
                        {
                          l: "Shorter",
                          x: "Make the document materially shorter while keeping all key facts.",
                        },
                        {
                          l: "Longer",
                          x: "Expand the document with more depth, examples and supporting detail.",
                        },
                        {
                          l: "Simplify",
                          x: "Simplify the language to a 9th-grade reading level without losing meaning.",
                        },
                        { l: "More formal", x: "Increase formality and remove colloquialisms." },
                        {
                          l: "Add visuals",
                          x: "Add tables, callouts, stat blocks and a Mermaid diagram where they help.",
                        },
                      ].map((a) => (
                        <button
                          key={a.l}
        ...
        <div className="rounded-2xl border border-border bg-card/60 p-5 shadow-[var(--shadow-elegant)] backdrop-blur">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <IconBtn onClick={redo} disabled={!future.length} title="Redo">
                <Redo2 className="h-3.5 w-3.5" />
              </IconBtn>
              <IconBtn
                onClick={() => setEditable((v) => !v)}
                title={editable ? "View only" : "Edit"}
              >
              ...
              <IconBtn onClick={copyMd} disabled={!output} title="Copy markdown">
                <Copy className="h-3.5 w-3.5" />
              </IconBtn>
              <IconBtn
                onClick={() => refine("Regenerate with a fresh structure.")}
                disabled={loading || !input.trim()}
                title="Regenerate"
              >
          ...
          <div className="mb-3 flex flex-wrap items-center gap-1 rounded-xl border border-border bg-background/40 p-1.5">
            <ToolBtn onClick={() => applyFormat("h1")} title="Heading 1">
              <Heading1 className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("h2")} title="Heading 2">
              <Heading2 className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("h3")} title="Heading 3">
              <Heading3 className="h-3.5 w-3.5" />
            </ToolBtn>
            <Sep />
            <ToolBtn onClick={() => applyFormat("bold")} title="Bold (first long word)">
              <Bold className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("italic")} title="Italic">
              <Italic className="h-3.5 w-3.5" />
            </ToolBtn>
            <Sep />
            <ToolBtn onClick={() => applyFormat("bullet")} title="Bulleted list">
              <List className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("ordered")} title="Numbered list">
              <ListOrdered className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("task")} title="Task list">
              <CheckSquare className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("quote")} title="Blockquote">
              <Quote className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("table")} title="Insert table">
              <TableIcon className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("code")} title="Code block">
              <Code2 className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("hr")} title="Divider">
              <Minus className="h-3.5 w-3.5" />
            </ToolBtn>
            <Sep />
            <ToolBtn onClick={() => applyFormat("callout-tip")} title="Tip callout">
              <Lightbulb className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("callout-warn")} title="Warning callout">
              <AlertTriangle className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("stats")} title="Stat row">
              <Hash className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("cards")} title="Card grid">
              <Square className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("pullquote")} title="Pull quote">
              <AlignLeft className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("kicker")} title="Kicker">
              <Type className="h-3.5 w-3.5" />
            </ToolBtn>
            <ToolBtn onClick={() => applyFormat("mermaid")} title="Mermaid diagram">
          ...
          {showPagePanel && <PageSettingsPanel page={page} update={updatePage} />}

          <div
            ref={previewWrapRef}
            className="preview-frame relative max-h-[820px] overflow-auto rounded-xl bg-black/20 p-4"
          >
            <div className="sticky top-0 z-10 mb-2 flex items-center justify-between gap-2 rounded-lg bg-background/85 p-1.5 backdrop-blur">
              <div className="flex items-center gap-1">
                ...
                <button
                  onClick={() => setZoom(1)}
                  className="min-w-[3.2rem] rounded-md border border-border bg-secondary px-2 py-1 text-[11px] tabular-nums hover:bg-muted"
                >
                  {Math.round(zoom * 100)}%
                </button>
                <IconBtn
              ...
              />
              <IconBtn onClick={toggleFullscreen} title="Toggle full screen">
...
function PageSettingsPanel({
  ...
  page: PageConfig;
  update: (p: Partial<PageConfig>) => void;
}) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Field label="Margins">
          <select
            value={page.margins}
            onChange={(e) => update({ margins: e.target.value as Margins })}
            className="select"
          >
            {MARGINS.map((m) => (
        ...
        <Field label="Border">
          <select
            value={page.border}
            onChange={(e) => update({ border: e.target.value as BorderStyle })}
            className="select"
          >
            {BORDER_STYLES.map((b) => (
        ...
        <Field label="Body font">
          <select
            value={BODY_FONTS.find((f) => f.value === page.bodyFont)?.id ?? "default"}
            onChange={(e) =>
              update({ bodyFont: BODY_FONTS.find((f) => f.id === e.target.value)?.value ?? "" })
            }
            className="select"
          >
            {BODY_FONTS.map((f) => (
        ...
        <Field label={`Body scale · ${page.bodyScale.toFixed(2)}×`}>
          <input
            ...
            max={1.3}
            step={0.05}
            value={page.bodyScale}
            onChange={(e) => update({ bodyScale: Number(e.target.value) })}
            className="w-full"
          />
        </Field>
        ...
        <Field label="Header">
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="Header text (default: today's date)"
              onChange={(e) => update({ headerText: e.target.value })}
              className="input"
              disabled={!page.showHeader}
            />
          </div>
        </Field>
        <Field label="Footer">
...
function Toggle({ on, onChange }: { on: boolean; onChange: (v: boolean) => void }) {
  return (
    ...
    >
      <span
        className={`h-4 w-4 rounded-full bg-foreground transition ${on ? "translate-x-4" : "translate-x-0.5"}`}
      />
    </button>
  );
}
...
function ControlRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <span className="mr-1 w-14 text-[10px] uppercase tracking-widest text-muted-foreground">
        {label}
      </span>
      {children}
    </div>
...
function Chip({
  ...
  return (
    <button
      onClick={onClick}
      className={`rounded-full border px-3 py-1 text-xs transition ${active ? "border-primary bg-primary/15 text-primary" : "border-border text-muted-foreground hover:text-foreground"}`}
    >
      {children}
    </button>
  );
...
function IconBtn({ children, ...rest }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button
      {...rest}
      className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-secondary px-2.5 py-1.5 text-xs hover:bg-muted disabled:opacity-40"
    >
      {children}
    </button>
  );
